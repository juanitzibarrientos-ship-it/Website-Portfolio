...
const words = ["Juanito","Web Developer","Frontend Designer","E-Commerce Builder"];
let i = 0;
let j = 0;
let currentWord = "";
...
function addItemToCart(name,price){ cart.push({name:name,price:price}); }
...
total += item.price;
cartList.append(`<li>${item.name} - $${item.price.toFixed(2)}<button class="remove-item btn btn-sm btn-danger ms-2" data-index="${index}">x</button></li>`);
});
